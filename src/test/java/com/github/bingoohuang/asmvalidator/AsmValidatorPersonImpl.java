...
public class AsmValidatorPersonImpl implements AsmValidator<Person3> {
    public AsmValidateResult validate(Person3 bean) {
        ...
        if (isBlank(age))
            result.addError(new ValidatorError("name", age, "不能为空"));

        if (!argNull && age.length() > AsmConsts.DEFAULT_MAX_SIZE) {
            result.addError(new ValidatorError("age", age, "长度超过" + AsmConsts.DEFAULT_MAX_SIZE));
        }

        if (argNull || age.length() != 3) {
            result.addError(new ValidatorError("age", age, "长度不等于3"));
        }
        ...
        if (isBlank(addr))
            result.addError(new ValidatorError("addr", addr, "不能为空"));
        if (addr != null && addr.length() > AsmConsts.DEFAULT_MAX_SIZE) {
            result.addError(new ValidatorError("addr", addr, "长度超过" + AsmConsts.DEFAULT_MAX_SIZE));
        }

        if (addrNull || addr.length() < 150) {
            result.addError(new ValidatorError("addr", addr, "长度小于3"));
        }
